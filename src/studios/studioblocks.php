...
while($row = sqlsrv_fetch_array($studios)){
  $id = $row['id'];
  echo '<tr id="tr'.$id.'" onclick=\'javascript:FillBlockDetail('.$id.',0)\';>';
...
echo '<td valign="top">';
echo '<div id="blockdetail">';
echo '<div class="tableheadersmall medtextboldcentre" style="width:50%">';
echo '<br>Choose a Studio from the list<br><br>';
echo '</div>';
echo '</div>';
echo '</td>';
...
function FillBlockDetail (studioid, id) {
  $('#blockdetail').load( 'studios/calls/showstudioblock.php?studioid='+studioid+'&id='+id, function() { });
}
