...
if (isset($arrDuties)) {
  ...
  // ################################################################ First a div with the Duty names
    ...
    foreach ($arrDuties as $intDutyID => $arrDuty) {
      if ($canmodify == 1) {
	if ($intArchived==0) {
	  echo '<div class="listplaceholder-context-menu" id="'.$intDutyID.'" dutyid="'.$intDutyID.'" prevdutyid="'.$prevDutyID.'" style="background-color: #99bfe6; position: absolute; width:230px; height:'.($intDutyHeight - 1).'px; left:0px; top:'.($intCounter * $intDutyHeight).'px">';
	}
	else {
	  echo '<div class="listplaceholderarchived-context-menu" id="'.$intDutyID.'" dutyid="'.$intDutyID.'" prevdutyid="'.$prevDutyID.'" style="background-color: #99bfe6; position: absolute; width:230px; height:'.($intDutyHeight - 1).'px; left:0px; top:'.($intCounter * $intDutyHeight).'px">';
	}
      }
      else {
	if ($intArchived==0) {
	  echo '<div class="listplaceholderbasic-context-menu" id="'.$intDutyID.'" dutyid="'.$intDutyID.'" prevdutyid="'.$prevDutyID.'" style="background-color: #99bfe6; position: absolute; width:230px; height:'.($intDutyHeight - 1).'px; left:0px; top:'.($intCounter * $intDutyHeight).'px">';
	}
	else {
	  echo '<div class="listplaceholderarchivedbasic-context-menu" id="'.$intDutyID.'" dutyid="'.$intDutyID.'" prevdutyid="'.$prevDutyID.'" style="background-color: #99bfe6; position: absolute; width:230px; height:'.($intDutyHeight - 1).'px; left:0px; top:'.($intCounter * $intDutyHeight).'px">';
	}
      }
      echo  $arrDuty['DutyName'];
      echo '<br>';
      //echo FormatTime($arrDuty['StartTime']).'-'.FormatTime($arrDuty['EndTime']);
      echo '('.($arrDuty['Duration']/3600).' Hours)';
    ...
    foreach ($arrDuties as $intDutyID => $arrDuty) {
      ...
      if ($canmodify == 1) {
	if ($intArchived==0) {
	  echo '<div class="listplaceholder-context-menu" id="dutydet'.$intDutyID.'" dutyid="'.$intDutyID.'" prevdutyid="'.$prevDutyID.'" style="height:'.($intDutyHeight).'px; position:absolute; top:'.($intCounter * $intDutyHeight).'px">';
	}
	else {
	  echo '<div class="listplaceholderarchived-context-menu" id="dutydet'.$intDutyID.'" dutyid="'.$intDutyID.'" prevdutyid="'.$prevDutyID.'" style="height:'.($intDutyHeight).'px; position:absolute; top:'.($intCounter * $intDutyHeight).'px">';
	}
      }
      else {
	if ($intArchived==0) {
	  echo '<div class="listplaceholderbasic-context-menu" id="dutydet'.$intDutyID.'" dutyid="'.$intDutyID.'" prevdutyid="'.$prevDutyID.'" style="height:'.($intDutyHeight).'px; position:absolute; top:'.($intCounter * $intDutyHeight).'px">';
	}
	else {
	  echo '<div class="listplaceholderarchivedbasic-context-menu" id="dutydet'.$intDutyID.'" dutyid="'.$intDutyID.'" prevdutyid="'.$prevDutyID.'" style="height:'.($intDutyHeight).'px; position:absolute; top:'.($intCounter * $intDutyHeight).'px">';
	}
      }
      // Draw in the Times
      drawtimecells("2015-01-01", $earlieststart, $lateststart, $intHourWidth, $intDutyHeight-1);

     if ($arrDuty['Duration'] > 0) {
	$intWidth = intval((($arrDuty['EndTime'] - $arrDuty['StartTime']) / 3600) * $intHourWidth);
      $left = intval(((floor($arrDuty['StartTime']/3600))-$earlieststart) * $intHourWidth);
      ...
      echo ''.$arrDuty['DutyName'].'';
      echo '</div>';  
      //echo '<div class="boxed" style="overflow:hidden; width:'.$intJobWidth.'px; height:'.($intDutyHeight - 16).'px; position:absolute; left:'.$intJobLeft.'px; top:8px; background-color:'.$arrJob['JobBackColour'].'">';
      //echo $arrJob['Job'];            
      //echo '</div>'; 
...
function DeleteDuty(dutyid, prevdutyid, lastmoddate, action) {
    ...
    $.ajax({ 
		    ...
		    if (data.status == 'success') {
		      //all good
		      ListMasterDuties(1, prevdutyid, 0);
		    }
		    else {
...
$(function(){
  $.contextMenu({
    selector: '.listplaceholderbasic-context-menu', 
    items: { 
    "history": {
        name: "History",
        icon: "history",
...
$(function(){
  $.contextMenu({
    selector: '.listplaceholdermodify-context-menu', 
    items: { 
    "edit": {
        name: "Edit",
        icon: "edit",
...
$(function(){
  $.contextMenu({
    selector: '.listplaceholder-context-menu', 
    items: { 
    "edit": {
        name: "Edit",
        icon: "edit",
...
$(function(){
    $.contextMenu({
    selector: '.listplaceholderarchivedbasic-context-menu', 
    items: { 
    "history": {
        name: "History",
        icon: "history",
...
$(function(){
    $.contextMenu({
    selector: '.listplaceholderarchived-context-menu', 
    items: { 
    "history": {
        name: "History",
        icon: "history",
