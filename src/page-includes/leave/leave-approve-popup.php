...
$callingPagename = $_REQUEST['callpage'] ?? 'weekly';
	//print_r($_POST);
if (isset($_POST['submit']) || isset($_POST['cancelBtn']))  {
	//print_r($_POST);
    $arrApplicationId = [];
    ...
    if (!empty($_POST['PDLTime'])) {
        ...
        foreach ($_POST['PDLTime'] as $k => $v) {
            if (($v[0] != '') && ($v[1] != '') && ($v[0] != 0 || $v[1] != 0)) {
                ...
                $baseDate = DateTime::createFromFormat('D, jS M Y', $v[2]);
                ...
                $startDateTime->modify("+{$v[0]} seconds");
                $endDateTime->modify("+{$v[1]} seconds");
                $pdlLeaveDataArr[$inc]['LeaveStartTimeLocal'] = $startDateTime->format('Y-m-d H:i');
                $pdlLeaveDataArr[$inc]['LeaveEndTimeLocal'] = $endDateTime->format('Y-m-d H:i');
                $inc++;
            }
        }
    }
    ...
    $strUserName = isset($leaveApplicationDetail['FullName']) ? $leaveApplicationDetail['FullName'] : '';
    $last7day = date('Y-m-d', strtotime('-7 day', strtotime($leaveApplicationDetail['dDate'])));

    $strLoopDate = '';
    $rsRequests = GetAllLeaveTypesDetailsForApprove($leaveApplicationDetail['Login'], $last7day);
    ...
    foreach ($rsRequests as $row) {
        ...
        if ($row['ID'] == $intID) {
            $intLeaveYear = date("Y", strtotime($row['dDate'] . "-3 months"));
    ...
    foreach ($arrRequests as $indLeaveID => $arrRequest) {
        ...
        $isPartDayAllowed = $arrAllocations['Weeks'][$intWeek][$intDay]['IsPartDayAllowed'] ?? 0;
        $isLeaveApproved = $arrAllocations['Weeks'][$intWeek][$intDay]['IsLeaveApproved'] ?? 0;
        $isPartDayLeaveApplied = $arrAllocations['Weeks'][$intWeek][$intDay]['IsPartDayLeaveApplied'] ?? 0;
        $dutyDuration = $arrAllocations['Weeks'][$intWeek][$intDay]['Duration'] ?? 0;
        $dutyName = $arrAllocations['Weeks'][$intWeek][$intDay]['Duty'] ?? '';
        $dutyStartTime = $arrAllocations['Weeks'][$intWeek][$intDay]['DutyStartTime'] ?? 0;
        $dutyEndTime = $arrAllocations['Weeks'][$intWeek][$intDay]['DutyEndTime'] ?? 0;
        $pdlStartTime = $arrAllocations['Weeks'][$intWeek][$intDay]['LeaveStartTime'] ?? 0;
        $pdlEndTime = $arrAllocations['Weeks'][$intWeek][$intDay]['LeaveEndTime'] ?? 0;
        $leaveId = $arrAllocations['Weeks'][$intWeek][$intDay]['LeaveID'] ?? $indLeaveID;
        $dutyDurationExcBreakSec = $arrAllocations['Weeks'][$intWeek][$intDay]['DutyDurationExcBreak'] ?? 0;
        $dutyId = $arrAllocations['Weeks'][$intWeek][$intDay]['DutyID'] ?? 0;
        ...
        $emptyDutyClass='';
        if((isset($arrAllocations['Weeks'][$intWeek][$intDay]) && ($arrAllocations['Weeks'][$intWeek][$intDay]=='')) || ($dutyName == 'U') || ($dutyName == 'Sick') || ($dutyName == 'Leave') || ($dutyName == 'OFF Leave') || (($dutyStartTime == 0 && $dutyEndTime == 0))){
            $emptyDutyClass =  'emptyDutyCell';
        }
        echo '<td class="wordBreak ' . $sickClass . ' ' . $rightClickPartDayLeave . ' '.$emptyDutyClass.'" title="' . $text . '" data-leaveId="' . $leaveId . '" data-week="' . $intWeek . '" data-duty-starttime="' . $dutyStartTime . '" data-duty-endtime="' . $dutyEndTime . '" data-duty-pdlstarttime="' . $pdlStartTime . '" data-duty-pdlendtime="' . $pdlEndTime . '" data-duty-name="' . $dutyName . '" data-duty-duration="' . $dutyDurationExcBreakSec . '" data-duty-id="' . $dutyId . '" id="leaveTd' . $leaveId . '" data-popup-open-id="' . $_REQUEST['id'] . '" data-popup-open-week="' . $_REQUEST['week'] . '" data-duty-date="' . $arrRequest['Date'] . '">';
        if (isset($arrAllocations['Weeks'][$intWeek][$intDay])) {
        ...
        foreach ($arrAllocLeaveTypes as $intTypeID => $arrAllocLeaveType) {
            ...
            if ((isset($arrAllocations['Weeks'][$intWeek][$intDay]['IsPartDayLeaveApplied']) && ($arrAllocations['Weeks'][$intWeek][$intDay]['IsPartDayLeaveApplied'] == 1)) && ($arrAllocations['Weeks'][$intWeek][$intDay]['IsLeaveApproved'] == 0 || $arrAllocations['Weeks'][$intWeek][$intDay]['IsLeaveApproved'] == 1)) {
        ...
        $reasonId = $arrRequest['ReasonID'] ?? 0;
        echo '<select size="1" id="Reason' . $indLeaveID . '-' . $intTypeID . '" name="ExceptionalTypes[' . $indLeaveID . '-' . $intTypeID . '-' . $reasonId . ']" class="reason" style="width: 100%;"  ' . $disabled . '  data-attr="' . $indLeaveID . '-' . $intTypeID . '">';
        echo '<option value="0">-</option>';
        foreach ($arrExceptionalTypes as $dataExceptionalType) {

            if (isset($reasonId) && $reasonId == $dataExceptionalType['ID']) {
    ...
    echo '<input id="submit" name="submit" type="submit" value="Submit">&nbsp;&nbsp;</input><input id="cancelBtn" type="button" value="Cancel" onclick="cancel()" >&nbsp;&nbsp;</input>';
    echo '</div>';
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            if(response.status == true){
                                setTimeout(function() {
                                    let callerPage = $('#callerPage').val();
                                    openManagePopup(popupLeaveId,weekNum,callerPage);
                                    GetTabContent(8);
                                }, 1000);
                            }
...
function updatePopUp(leaveid, week) {
    $.ajax({
        ...
        success: function (data) {
            ...
            let callerPage = $('#callerPage').val();
            if(callerPage==1) {
              ShowLeaveWeeklyAdminByDate('<?php echo $strPassedDate ?>', <?php echo $intGroupID ?>);
            }else if(callerPage==2) {
              AdminShowLeaveWeekly(<?php echo $_POST['week'] ?>);
              refreshLeaveWeeklyTabData(<?php echo $_POST['week'] ?>,<?php echo $intGroupID ?>);
            } else {
              ShowYealyLeaves();
              refreshLeaveWeeklyTabData(<?php echo $_POST['week'] ?>,<?php echo $intGroupID ?>);
            }
...
function AdminApplication (applicationid, action) {
  var callerPage = $('#callerPage').val();
  $.ajax({
    url: 'page-includes/leave/leave-application-admin.php',
    type: 'POST',
...
function refreshLeaveWeeklyTabData(weekNumber = 0,grpId = 0){
    let weekNum = $.cookie('leaveWeeklyTabWeekNum');
    let groupId = $.cookie('leaveWeeklyTabGroupId');
    weekNum = (weekNum != undefined && weekNum > 0) ? weekNum:weekNumber;
    groupId = (groupId != undefined && groupId > 0) ? groupId:grpId;
