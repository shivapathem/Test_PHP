...
<div class="teamPayFilterBlock">
	<div class="grid box1">
		<div class="recordFilter recordFilter1">
			...
			<form name="leaversRecordFilter" id="leaversRecordFilter" onSubmit="return false;" >
				<?php
				...
				<input type="hidden" id="conrollerName" name="conrollerName" value="getLeaversRecord" >
		...
		<div class="recordFilter recordFilterBtn">
			<div class="recordFilterBox">
				<input name="filterRecords" id="filterRecords" type="button" value="Filter" onclick="$('#recordOffset').val(0); getLeaversRecord(true);">
				<input name="excludeRecords" id="excludeRecords" type="button" value="Exclude" onclick=" document.getElementById('filterType').value = 0; $('#recordOffset').val(0); getLeaversRecord(true);" <?php if($requestData['filterType'] == "0"){echo 'style="background-color:#ccc"';} ?>>
				<input name="clearRecords" id="clearRecords" type="button" value="Clear" onclick="clearLeaversRecordsFilter();">
				<div style="display: inline-flex;float: right;left: 250%; position: absolute; width:300%">
					...
					<input type="button" onClick="$('#recordOffset').val(parseInt($('#recordOffset').val()) - 1);getLeaversRecord(true);" <?php if($requestData['recordOffset'] == 0){echo 'disabled';} ?> value="&lt;&lt;" > 
					<input type="button" onClick="$('#recordOffset').val(parseInt($('#recordOffset').val()) + 1);getLeaversRecord(true);" <?php if(count($data[0]) < 100){echo 'disabled';} ?> value="&gt;&gt;" >
				</div>
			</div>
		</div>
	</div>
</div>
<div class="teamPayRecordSection" id="leaversRecordContainer" onScroll="saveScrollPosForLeaversRecord();" >
...
<table id="leaversRecordEntries" class="oddevenclass tablesmall" style="width:100%">
	...
	<?php 
		...
		foreach($data[0] as $dataVal)
		{
			...
			$paymentTypeStyle = '';
			if($dataVal['A7_PaymentTypeName'] != $dataVal['PaymentTypeName'])
			{
			...
			if($rowDisabledCount == 0)
			{
				$disabledStr	=	'disabled';
			}
			$onHoldClass		=	'';
			$dataVal['comments'] = $dataVal['comments'] ? $dataVal['comments'] : '';
			if($dataVal['comments'] != 'Approved' && strlen($dataVal['comments']) > 0)
			{
				$onHoldClass	=	'onHoldClass';
			}
			$dataVal['A7_EFT'] = $dataVal['A7_EFT'] ? number_format($dataVal['A7_EFT'], 3) : 0.000;
			$dataVal['EFT'] = number_format($dataVal['EFT'], 3);
			$leaveDate = $dataVal['LeaveDate'] ? date('d-m-Y', strtotime($dataVal['LeaveDate'])) : '';
	?>
		<tr class="<?php echo $rowSeperatorClass. " $onHoldClass"; ?>" >
			...
			<td><?php echo isset($dataVal['UserID']) ? $dataVal['UserID'] : ''; ?></td>
			...
			<td style="white-space: nowrap;"><?php echo $leaveDate; ?></td>
			<td style="white-space: nowrap;"><?php echo $dataVal['StartDate'] ? date('d-m-Y', strtotime($dataVal['StartDate'])) : ''; ?></td>
			<td style="white-space: nowrap; <?php echo $endDateStyle; ?>"><?php echo $dataVal['A7_EndDate'] ? date('d-m-Y', strtotime($dataVal['A7_EndDate'])) : ''; ?></td>
			<td style="white-space: nowrap; <?php echo $endDateStyle; ?>"><?php echo $dataVal['EndDate'] ? date('d-m-Y', strtotime($dataVal['EndDate'])) : ''; ?></td>
			...
			<td <?php echo $paymentTypeStyle; ?>><?php echo $dataVal['A7_PaymentTypeName']; ?></td>
			<td <?php echo $paymentTypeStyle; ?>><?php echo $dataVal['PaymentTypeName']; ?></td>
...
<input type="hidden" id="formName" name="formName" value="leaversRecords" >
</form>
</div>
