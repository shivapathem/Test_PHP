...
if (!empty($rsDuties)) {
  ...
  if (!empty($arrDuties)) {
    ...
    foreach ($arrDuties as $intDutyID => $arrDuty) {
      ...
      $intWidth = 500;
      $left = 0;
      echo '<div class="'.$classname.'" dutyid="'.$intDutyID.'" dutyname="'.$arrDuty['DutyName'].'" dutytypeid="4" style="border: 1px solid #C0C0C0; text-align:center; width: '.$intWidth.'px; height:'.($intDutyHeight - 8).'px; position:absolute; left:'.$left.'px; top:4px; background-color:#'.strval($arrDuty['backcolour']).'; color:#'.strval($arrDuty['forecolour']).';">';
