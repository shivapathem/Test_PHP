USE [Allocate7]
GO
/****** Object:  StoredProcedure [dbo].[usp_AssignToUnallocateFromMasterDuty]    Script Date: 17/10/2024 13:31:57 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE OR ALTER   PROCEDURE [dbo].[usp_AssignToUnallocateFromMasterDuty]
@pweekNumber		 INT,
@pDutyID			 INT,
@pTeamID             INT,
...
BEGIN
   ...
   DECLARE  @viday            INT
   DECLARE  @createdutycnt    INT
   DECLARE  @removedutycnt    INT
   DECLARE  @vWeekNumber      INT
   DECLARE  @vAllocationID    INT
   DECLARE  @vname            VARCHAR(50)
   DECLARE  @vuserID          INT
   DECLARE  @vTeamID          INT
   DECLARE  @vSQL             VARCHAR(MAX)
   DECLARE  @vHistory         NVARCHAR(MAX)
   DECLARE  @vstartweek       INT
   DECLARE  @vendweek         INT
   DECLARE  @vdutyname        NVARCHAR(60)
   DECLARE  @vdutycolourid    INT
   DECLARE  @vduration        INT
   DECLARE  @vstarttime       INT
   DECLARE  @vendtime         INT
   DECLARE  @vbackcolour      NCHAR(10)
   DECLARE  @vforecolour      NCHAR(10)
   DECLARE  @vbreaktime       INT
   DECLARE  @aldutyname       NVARCHAR(60)
   DECLARE  @alstarttime      INT
   DECLARE  @alendtime        INT
   DECLARE  @albackcolour     NCHAR(10)
   DECLARE  @alfontcolour     NCHAR(10)
   DECLARE  @alstartdate      DATETIME
   DECLARE  @alenddate        DATETIME
   DECLARE  @aldutybreaktime  INT
   DECLARE  @aldutycolorid    INT
   DECLARE  @alduration       INT
   DECLARE  @alid             INT
   DECLARE  @aldutydate       DATETIME
   DECLARE  @dutyProgramId    INT,
            @dutyProgramId2   INT,
			@dutyProgramId3   INT,
			@dutyProgramId4   INT,
			@dutyProgramId5   INT,
   ...
   DECLARE  @vTimeChangeFlag  INT = 0
   DECLARE  @voldcolourname   NVARCHAR(50)
   DECLARE  @vnewcolourname   NVARCHAR(50)
   DECLARE  @year             VARCHAR(4)
   DECLARE  @week             VARCHAR(4)
   DECLARE  @Message          VARCHAR(100)
   DECLARE  @NewLabel         NVARCHAR(50)
   DECLARE  @OldLabel         NVARCHAR(50)

		SET @year = SUBSTRING(cast(@pweekNumber as varchar), 1, 4)
		SET @week = SUBSTRING(cast(@pweekNumber as varchar), 5, 6)

select @vname = Forename+' '+Surname
from StaffDetails  (NOLOCK)
...
END;
				...
				IF NOT EXISTS ( SELECT TOP 1 WeekNumber
				                 ...
				                 WHERE WeekNumber >= @pweekNumber and SchedulingTeamId = @pTeamID  )
BEGIN
...
BEGIN
					     SET @updateflag = 1
						 SET @vSQL = @vSQL+' backcolour = '+@vforecolour+', '
END
...
BEGIN
						 ...
						 SET @vSQL = @vSQL+' UpdatedBy = '+ CAST(@vuserID AS VARCHAR)
						                                   +', UpdatedDate = getutcdate() WHERE ID = '
														   +cast(@alid as varchar)
						 EXEC (@vSQL)
...
DELETE History WHERE AttributeID in (
			                                       SELECT ID
...
DELETE FROM Allocations_jobs
WHERE ID IN ( SELECT  ID FROM #TempAllocationsJobs )
...
FROM Allocations_jobs AJ1
         INNER JOIN Allocations AL1 (NOLOCK) ON AL1.ID = AJ1.AllocationID
...
WHERE al1.weeknumber >= @pweeknumber
  AND al1.schedulingteamid = @pteamId
  AND al1.masterdutyid = @pDutyID
  AND ISNULL(al1.SchedulingPersonID,0) = 0
  AND MJ1.IsActive = 1
...
FROM   (
       ...
       (  SELECT id
          FROM   historytypes
          WHERE  historytype='AllocationJobs') ht
...
SET AJ.JobName = MJ.JobName,
    ...
    INNER JOIN MasterDutiesMasterJobs_LINK as mdmj (NOLOCK) on MJ.MasterJobID = mdmj.MasterJobID
    INNER JOIN Allocations AL (NOLOCK) on AL.ID = AJ.AllocationID AND AL.MasterDutyId=mdmj.MasterDutyID
    LEFT JOIN LINK_MasterJobs_Programmes MLP (NOLOCK) ON MJ.MasterJobID=MLP.MasterJobID
    LEFT JOIN Programmes PG (NOLOCK) ON MLP.ProgrammeID=PG.ID
    LEFT JOIN REF_MasterJobColours JC (NOLOCK) ON MJ.MasterJobID=JC.MasterJobID and JC.IsActive=1
WHERE MJ.IsActive=1
         ...
         FROM   (SELECT masterdutyid,
                 ...
                 FROM   (SELECT masterdutyid,
                         ...
                         WHERE  MasterDutyID = @pDutyID
                           AND @pweekNumber BETWEEN startweek AND endweek ) MD
                     UNPIVOT (dutycount
                 ...
                 WHERE  dutycount > 0) AS FMD
                    FULL JOIN (SELECT AP.SchedulingTeamID,
                               ...
                               FROM Allocations AL (NOLOCK)
                                        ...
                                        inner join Allocations_published_weeks AP (NOLOCK) ON AL.AllocationID = AP.AllocationID
                               WHERE ap.weeknumber between md.startweek AND md.endweek
                                 AND AP.SchedulingTeamId = @pTeamID
                                 AND AL.MasterDutyID = @pDutyID
                                 AND AP.WeekNumber >= @pweekNumber
                               group by AP.SchedulingTeamID,ap.WeekNumber, al.iday, md.MasterDutyID,AP.allocationID
...
SELECT 0                     AS allocateinstanceid,
       ...
       cast(CASE
                WHEN md.starttime IS NOT NULL THEN Concat(
                        ...
                        ' ', (
                            RIGHT(
                            '0' + Cast(Cast( case when MD.StartTime >= 86400 then ( MD.StartTime - 86400) else MD.StartTime end AS INT)
                            / 3600
                            AS VARCHAR )
                            , 2)
                            +
                            ':'
                            + RIGHT('0' +
                            Cast((Cast( case when MD.StartTime >= 86400 then ( MD.StartTime - 86400) else MD.StartTime end AS INT) / 60
                            ) % 60
                            AS VARCHAR), 2)
                            + ':'
                            + RIGHT('0' +
       ...
       cast(CASE
                WHEN md.endtime IS NOT NULL THEN Concat(Format (CASE
                                                                    WHEN
                                                                            ( case when MD.EndTime >= 86400 then (MD.EndTime - 86400) else MD.EndTime end )
                                                                            < ( case when MD.StartTime >= 86400 then ( MD.StartTime - 86400) else MD.StartTime end ) THEN
                                                                        DATEADD(DAY,1,td3.dutydate)
                                                                    ELSE dutydate
                                                                    END, 'yyyy-MM-dd'), ' '
                    , (
                                                            RIGHT('0' + Cast(Cast( case when MD.EndTime >= 86400 then (MD.EndTime - 86400) else MD.EndTime end AS
                                                            INT) /
                                                            3600 AS VARCHAR ), 2)
                                                            + ':'
                                                            + RIGHT('0' + Cast((Cast( case when MD.EndTime >= 86400 then (MD.EndTime - 86400) else MD.EndTime end
                                                            AS INT)
                                                            / 60) % 60 AS VARCHAR), 2)
                                                            + ':'
                                                            + RIGHT('0' + Cast(Cast( case when MD.EndTime >= 86400 then (MD.EndTime - 86400) else MD.EndTime end
...
FROM   Allocations AL (NOLOCK),
       (SELECT id
        FROM   HistoryTypes (NOLOCK)
        WHERE  historytype = 'AllocationDuty') HT
...
SELECT 'Assigned as Unallocated Duty : ('
           + cast(@vdutyname as varchar)
           +') Starting From Week Number '+@week+'/'+@year
           AS successMessage,
       1 AS spStatus
