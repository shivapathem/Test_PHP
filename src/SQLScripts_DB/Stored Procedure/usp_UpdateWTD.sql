USE [BBCSchedules]
GO
/****** Object:  StoredProcedure [dbo].[usp_UpdateWTD]    Script Date: 3/24/2026 9:28:00 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	    ...
	    WHERE AL_SchedulingTeamID = @pteamId
		 ...
		 SELECT AP.ASP_SchedulingPersonID,
		  ...
		  WHERE AL.AL_SchedulingTeamID = @pteamId
			AND TD.dDateTime between DATEADD (DAY, -21, @DutyDate) AND DATEADD(DAY,21,@DutyDate)
			AND AP.ASP_SchedulingPersonID = @schedulingpersonid
			AND AL.AL_Status <> 9
			AND ALA.AL_Status <> 9
    ...
    IF EXISTS ( SELECT 1 
				 ...
				 WHERE AL.DutyDate between @DutyDate and DATEADD(DAY,1, @DutyDate)
				   AND ( IsUnderElevenBreak = 1 OR IsUnderElevenBreakOverride = 1)
