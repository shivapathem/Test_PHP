...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[usp_InsAddStaffToRestrictedRequestType]') AND type in (N'P', N'PC'))
SET @strSQL = N'ALTER '
ELSE
SET @strSQL = N'CREATE '

--Execute the script statement
Set @strSQL= @strSQL + N' PROCEDURE [dbo].[usp_InsAddStaffToRestrictedRequestType]
...
BEGIN
	...
	BEGIN TRY
		...
		INSERT INTO       RequestTypesStaffLink(Login, RequestTypeID)
        VALUES            (@strStaffLogin, @intRequestType)

	   set @History = ''Record updated by '' +  @Fullname + '' on '' +  CONVERT(VARCHAR, GETDATE(), 103) + '' at '' + CONVERT(VARCHAR(5),getdate(),108)   + ''.<br>Access to Request Type '''''' + @RequestTypeName + '''''' belonging to '''''' + @RequestGroupName + '''''' was added.<hr>''
