USE [Allocate7]
GO
/****** Object:  StoredProcedure [dbo].[usp_get_ReadAllocationsForLeave]    Script Date: 16/03/2022 14:33:30 ******/
SET ANSI_NULLS ON
GO
...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[usp_get_ReadAllocationsForLeave]') AND type in (N'P', N'PC'))
SET @strSQL = N'ALTER '
ELSE
SET @strSQL = N'CREATE '

--Execute the script statement
Set @strSQL= @strSQL + N' PROCEDURE [dbo].[usp_get_ReadAllocationsForLeave]
@StartWeek INT,
@EndWeek INT,
...
BEGIN
	...
	SELECT @startDate=dDateTime From TimeDimension where ixYearWeek=@StartWeek
	SELECT @endDate=dDateTime From TimeDimension where ixYearWeek=@EndWeek
	...
	LEFT JOIN StaffDetails sd (NOLOCK) on sd.StaffID = sp.StaffDetailsID AND sd.NetLogin = @NetLogin
	...
	A.SchedulingPersonID = @SchedulingPersonId
	 And spl.isActive = 1 and isnull(convert(datetime,spl.EndDate,110),''9999-01-01'') >= convert(datetime,convert(varchar,@endDate,110),110)
						 and convert(datetime,spl.StartDate,110) <= convert(datetime,convert(varchar(30),@startDate,110),110) 
