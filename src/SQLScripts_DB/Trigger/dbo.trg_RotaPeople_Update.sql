...
IF EXISTS (SELECT * FROM sys.triggers WHERE [name] = N'trg_RotaPeople_Update' AND [type] ='TR')
SET @strSQL = N'ALTER '
ELSE
SET @strSQL = N'CREATE '

--Execute the script statement
Set @strSQL= @strSQL + N' TRIGGER [dbo].[trg_RotaPeople_Update] on [dbo].[RotaPeople] AFTER INSERT,UPDATE  AS BEGIN
Set nocount on;
Declare @RotaId int;
